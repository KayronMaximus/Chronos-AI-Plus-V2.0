...
// ============================================================================
// =========================================================
// FUNÇÃO: NOME DO USUÁRIO = NOME DO SISTEMA
// =========================================================
function sincronizarNomeSplash() {
  // 1. Pega o nome salvo no localStorage. Se não tiver nada, usa "CHRONOS" padrão.
  ...
  // 3. Substitui o texto "CHRONOS" pelo NOME DO USUÁRIO (em Maiúsculo)
  if (tituloSplash) {
    // Exemplo: Se nomeSalvo for "Maximus", vira "MAXIMUS AI"
  ...
  try {
    ...
    if (response.ok) {
      const dataDisplay = dataHoraEscolhida
        ? new Date(dataHoraEscolhida).toLocaleString("pt-BR")
        : "Agora";
...
function atualizarDashboard() {
  ...
  listaHome.innerHTML = pendentes.length
    ? ""
    : "<p style='color:#888'>Tudo limpo!</p>";
...
function atualizarGraficoTarefas() {
  ...
  document.getElementById("texto-progresso").innerText = total
